...
function BugReports (): JSX.Element {
  ...
  const [bug, setBugs] = React.useState<BugReport[]>([])
  ...
  return (
    <div id="admin-bur-reports-page" style={{ marginTop: 15 }}>
      ...
      <Button
        ...
        <Typography variant='h3'>All Bug Reports</Typography>
        <Box sx={{ flexGrow: 1 }}>
          {/* Data table for decks */}
          { isLoading ? <CircularLoader /> : <DataTable data={bug} onClick={(id) => { navigate(`${String(id)}`) }} /> }
        </Box>
      </Container>
    </div>
  )
