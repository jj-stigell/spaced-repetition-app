...
const findKanjiById = async (kanjiId, langId, includeAccountCard) => {
  try {

    const selectedLanguage = langId ? langId : 'en';
    ...
    const kanji = await models.Kanji.findByPk(kanjiId, {
      where: {
        'id': kanjiId
      },
      subQuery: false,
    ...
    return await models.Card.findByPk(kanjiId, {
      where: {
        'id': 'kanji',
        'active': true
      },
...
const fetchAllKanji = async (accountId, langId) => {
  try {
    const selectedLanguage = langId ? langId : 'en';
